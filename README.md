...
- **Build mode**: Design forms by adding, moving, and resizing fields overlaid on top of the PDF.
- **Field mirroring**: Existing PDF form fields can be mirrored and edited in build mode.
- **Field assignment**: Assign fields to participants (e.g., landlord/tenant) and enforce visibility/editability in edit mode.
- **Multi-mode support**: View (read-only), Edit (fill forms), and Build (design forms) modes.
...
| mode                 | `"view" \| "edit" \| "build"`, optional                                                                               | Controls behavior. Default: `"edit"`. In build mode, fields can be added/edited; in edit mode, users fill fields; in view mode, read-only.                                                                                                                                                                                                                                                                                                                         |
...
| participants         | `{ id: string; label: string; role?: "landlord" \| "tenant" }[]`, optional                                            | Participants available to assign to fields in build mode.                                                                                                                                                                                                                                                                                                                                                                                                          |
| activeParticipantId  | `string`, optional                                                                                                    | In edit mode, only fields assigned to this participant are editable.                                                                                                                                                                                                                                                                                                                                                                                               |
| unassignedVisibility | `"readonly" \| "hidden"`, optional                                                                                    | How fields not assigned to the active participant behave in edit mode. Default: `"readonly"`.                                                                                                                                                                                                                                                                                                                                                                      |
| onBuildSave          | `(pdfBytes: Uint8Array, buildSchema: BuildModeField[], fieldAssignments: Record<string, string[]>) => void`, optional | Receives the designed field schema, field assignments, and PDF in build mode.                                                                                                                                                                                                                                                                                                                                                                                      |
| fieldAssignments     | `Record<string, string[]>`, optional                                                                                  | Override field assignments for edit mode. If omitted, assignments are automatically extracted from PDF metadata. Only needed if you want to override the stored assignments.                                                                                                                                                                                                                                                                                       |
...
### Build Mode - Design Forms

```typescript
<PDFEditor
  src="/template.pdf"
  mode="build"
  onBuildSave={(pdfBytes, buildSchema) => {
    // Save both the PDF and the field schema
    console.log("Field schema:", buildSchema);
  }}
/>
```

### Field Assignment - Lease Signing Workflow

**Note**: Field assignments are automatically stored in the PDF metadata, so you don't need to manage them separately in your database.

```typescript
// Landlord builds the lease form
<PDFEditor
  src="/lease.pdf"
  mode="build"
  participants={[
    ...
    { id: "tenant2", label: "Tenant 2", role: "tenant" },
  ]}
  onBuildSave={(pdfBytes, schema, fieldAssignments) => {
    // Persist the schema and field assignments
    saveLeaseSchema(schema);
    saveFieldAssignments(fieldAssignments);
  }}
/>

// Later, tenant fills their assigned fields
<PDFEditor
  src="/lease.pdf"
  mode="edit"
  activeParticipantId="tenant1"
  unassignedVisibility="readonly"
  onSave={(pdfBytes, values) => {
